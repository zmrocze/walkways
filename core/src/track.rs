...
use uom::{si::{f64::{Velocity, Acceleration, Length}, velocity::{kilometer_per_hour, centimeter_per_second}}, ConstZero};

use crate::{platform::Calculate, common::{InputParams, RunStatus, TrackPosition}};
...
impl TrackSection {
  fn new(beginning: TrackPosition, section_type:SectionType) -> TrackSection {
    TrackSection { beginning: beginning, section_type: section_type }
  }
}
...
impl Track {
  ...
  pub const fn standard_track(
    ...
    let deceleration_distance = 
      0.5*(max_vel + min_vel)*(min_vel - max_vel)/max_deceleration;
    ...
    let first_slowingpoint = new_track_position(
      0.5 * track_length - deceleration_distance
    );
    let track_half = new_track_position(
      0.5 * track_length
    );
    // Analogous to first_slowingpoing but in the second half of the track (underground half)
    let second_slowingpoint = new_track_position(
      track_length - deceleration_distance);
    let sections = vec![
      TrackSection::new(new_track_position(Length::ZERO), SectionType::MaxSpeed),
      TrackSection::new(first_slowingpoint, SectionType::Decelerate { exit_velocity: min_vel }),
      TrackSection::new(track_half, SectionType::MaxSpeed),
      TrackSection::new(second_slowingpoint, SectionType::Decelerate { exit_velocity: min_vel }),
      ];
      ...
      RunStatus::Start => {
        ...
        match self.track.sections[section_ind].section_type {
          ...
          SectionType::Decelerate { exit_velocity } => {
            let dist_to_section_exit = input.position.distance_to(self.track.sections[(section_ind + 1) % n].beginning);
            ...
            let current_desired_vel = (exit_velocity*exit_velocity - 2.0*self.track.max_deceleration*dist_to_section_exit).sqrt();
