...
pub fn run_platform_controller<'a, I, O, Calc: Calculate<I,O>, Mon: Monitor<I>, Conn : CentreConnection>(
  ...
  loop {
    ...
    platform.setter.set(
      platform.calculate_params.calculate(&run_status, &inp)
    );
...
fn run_platform<'a, I, O, Calc: Calculate<I, O> + Send, Mon: Monitor<I> + Send>(
  ...
  our_address: std::net::SocketAddr)
  ...
  let connection = runtime.block_on(
